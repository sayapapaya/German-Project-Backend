...
class ArticleBlob:
    ...
    def get_relative_frequency_of_nouns(self):
        ...
        number_nouns_in_blob = count_words_in_blob_if_tag_meets_criteria(
            self.blob, is_noun)
        return float(number_nouns_in_blob) / len(self.blob.words)
...
def is_noun(tag_string):
    """"Returns True if the tag_string is one of the POS noun tags"""
    result = True if tag_string in POS.POS_tags.noun_tags else False
    return result

def is_verb(tag_string):
    """"Returns True if the tag_string is one of the POS verb tags"""
